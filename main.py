...
def draw_confusion_matrix(cf_matrix, sub, results_path):
    ...
    # display_labels = ['Right hand','Foot'] #dataset IVa label
...
def test(model, Dataset_config, results_path, allRuns = True):
    ...
    for sub in range(n_sub): # (num_sub): for all subjects, (i-1,i): for the ith subject.
        ...
        AUC_bestRun[sub] = roc_auc_score(labels, y_pred_onehot,multi_class='ovr') # IIb y_pred
...
def getModel(model_name):
    ...
    model = MMANet(
        # Dataset parameters
        n_classes=4, # IIa /IIIa
        # n_classes = 2, #  dataset IIb
        in_chans=22, # dataset IIa
        # in_chans=3, # IIb
        # in_chans = 60, # IIIa
        in_samples=1125,
